...
RE:CH-RZL-RUST 是一个使用 Rust 重写的 Rizline 风格音游谱面播放器与离线渲染器。项目基于 macroquad 实现画面渲染，支持读取谱面 JSON、播放或选择背景音乐、显示音符与判定线动画，并可通过 FFmpeg 导出带音频的竖屏视频。
...
本项目是对原网页音游播放器的 Rust 迁移版本，目标是提供更独立的本地播放和离线录制 / 渲染能力。原网页项目目录 ch-rzl/ 可作为参考资源目录保留，但不纳入当前 Rust 项目的 Git 跟踪。
